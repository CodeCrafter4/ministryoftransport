...
const AdminDashboard = () => {
  const [stats, setStats] = useState({
    ...
    transport: { routes: 0, activeRoutes: 0 }
  })
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [usersRes, vehiclesRes, licensesRes, transportRes] = await Promise.allSettled([
      ...
      if (transportRes.status === 'fulfilled' && transportRes.value.ok) {
        const transportData = await transportRes.value.json()
        setStats(prev => ({
          ...prev,
          transport: transportData.stats || { routes: 0, activeRoutes: 0 }
        }))
      }
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-soft p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Transport Routes</p>
              <p className="text-3xl font-bold text-gray-900">{stats.transport.routes}</p>
              <p className="text-sm text-green-600 mt-1">
                <span className="font-medium">{stats.transport.activeRoutes}</span> active
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-soft p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">Transport Routes</h3>
                  <p className="text-sm text-gray-600">Manage public transport</p>
                </div>
              </Link>
            </div>
          </div>
